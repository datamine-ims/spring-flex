...
import org.springframework.flex.hibernate5.HibernateProxyConverter;
import org.springframework.flex.hibernate5.PersistentCollectionConverterFactory;
...
//    }
    protected void extractHibernateMetadata(SessionFactory sessionFactory) {
        ...
        // Populate classMetadata set
        Map<String, EntityPersister> entityPersisters = sfi.getMetamodel().entityPersisters();
        for (EntityPersister persister : entityPersisters.values()) {
            if (persister instanceof ClassMetadata) {
                this.classMetadata.add((ClassMetadata) persister);
            }
        }

        // Populate collectionMetadata set
        Map<String, CollectionPersister> collectionPersisters = sfi.getMetamodel().collectionPersisters();
        for (CollectionPersister persister : collectionPersisters.values()) {
            if (persister instanceof CollectionMetadata) {
                this.collectionMetadata.add((CollectionMetadata) persister);
            }
        }
